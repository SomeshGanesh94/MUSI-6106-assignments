...
public:
    ...
    */
    void put(const T* ptNewBuff, int iLength)
    {
        ...
        {
            memcpy(&m_ptBuff[m_iWriteIdx], ptNewBuff, (m_iBuffLength - m_iWriteIdx) * sizeof(T));
            memcpy(&m_ptBuff[0], &ptNewBuff[m_iBuffLength - m_iWriteIdx], (iLength - (m_iBuffLength - m_iWriteIdx)) * sizeof(T));
        }
        else
        {
            memcpy(&m_ptBuff[m_iWriteIdx], ptNewBuff, iLength * sizeof(T));
        }
    }
